...
function curlPost(url: string, body?: string): string {
  const data = body === undefined ? "" : ` -H 'content-type: application/json' --data ${shellQuote(body)}`;
...
function reproCommand(baseUrl: string, incident: Incident, sample: LogEvent): string {
  ...
  const body = sample.level === "warn" ? discountBody() : missingCustomerBody();
  return curlPost(`${baseUrl}/orders`, body);
}
...
async function reproduceShip(
  state: TriageState,
  baseUrl: string,
  command: string,
): Promise<NonNullable<IncidentTriage["repro"]>> {
  const before = { offset: Bun.file(state.logPath).size };
  ...
  await Bun.sleep(120);
  const newLogs = await readNewEvents(state.logPath, before);
...
async function reproduceIncident(
  state: TriageState,
  incident: Incident,
): Promise<NonNullable<IncidentTriage["repro"]>> {
  const baseUrl = state.config?.baseUrl.replace(/\/$/, "") ?? "http://localhost:3000";
  const sample = incident.sampleEvents[0];
  if (!sample) return { reproduced: false, command: "", evidence: "Incident has no sample log." };
  const command = reproCommand(baseUrl, incident, sample);

  try {
    if (incident.fingerprint.route === "POST /orders/:id/ship") {
      return await reproduceShip(state, baseUrl, command);
    }
    ...
    return { reproduced: false, command, evidence: "No HTTP reproduction is known for this route." };
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    return {
      reproduced: false,
      command,
      evidence: `Service unreachable or request failed: ${message}`,
    };
  }
}
...
export async function reproduceNode(state: TriageState): Promise<Partial<TriageState>> {
  ...
  for (const item of state.triage ?? []) {
    triage.push({ ...item, repro: await reproduceIncident(state, item.incident) });
  }
